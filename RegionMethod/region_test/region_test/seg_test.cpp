...
//  seg_test
//
//  Created by qiuxinxin on 15/7/17.
...
int main(int argc, char* argv[])
{
    ...
    IPL image_border = cvCreateImage( cvSize( image1 -> width+2, image1 -> height+2 ), IPL_DEPTH_8U, image1->nChannels);//创造一个图像header并分配给图像数据（图像宽高，图像元素的比特数，这里为8，每像素的通道数），->可以看到图像所有的性质 argv为输入各参数的名称
    ...
    int ai[4]={0,0,image2.rows-N,image2.rows-N},aj[4]={0,image2.cols-N,0,image2.cols-N};
    int bi[4]={N,N,image2.rows,image2.rows},bj[4]={N,image2.cols,N,image2.cols};
    //    int M=40000;
    //    Mat trainingData2(0,5,CV_32FC1);//负样本，取图像4个角100x100像素
    //    Mat trainingDataMat2;
    //    int ai[4]={0,0,image2.rows-100,image2.rows-100},aj[4]={0,image2.cols-100,0,image2.cols-100};
    //    int bi[4]={100,100,image2.rows,image2.rows},bj[4]={100,image2.cols,100,image2.cols};
    ...
    predictDataMat1=predictData1.reshape(0,image2.rows*image2.cols);
    //    cout<<predictDataMat.size()<<endl;
    //    Mat predictDataMat=Mat(image2.rows*image2.cols, column, CV_32FC1);
    //    for (i=0; i<column; i++)
    //    {
    //        double min=1, max=1;
    ...
    //                image2.at<Vec3b>(i,j)  = green;
    //            else if (responseMat.at<float>(i,j) == -1)
    //                image2.at<Vec3b>(i,j)  = blue;
    //        }
    //
    //    }
    ...
    for (i=0;i<image2.rows;i++)
    {
        ...
        {
            if (responseMat.at<float>(i,j)== 1)
                image2.at<Vec3b>(i,j)  = white;
            else if (responseMat.at<float>(i,j) == -1)
                image2.at<Vec3b>(i,j)  = black;
        }
    ...
    imwrite(shortstring,image2);
    
    //    imwrite("/Users/qiuxinxin/temp/角毛藻显微图像/TestOpencv/result-nonormal.png", image2);
    //    imshow("SVM Simple Example", image2); // show it to the user
    //    waitKey(0);
